...
def process_batch_JRS_trig(jrs_tensor, q_0,qd_0,joint_axes):
    ...
    for i in range(n_joints):
        ...
        JRS_batch_zono = batchZonotope(jrs_tensor[closest_idx]) #center=(100,6),generators=(100,12,6),Z=(center,generators)=(100,13,6)
        ...
        JRS_batch_zono = A@JRS_batch_zono.slice(kv_dim,qd_0[i:i+1]) #Z=(100,12,6),generators=(100,11,6)
        ...
        '''
        R_temp= gen_batch_rotatotope_from_jrs_trig(PZ_JRS,joint_axes[i]) #([3*3])

        PZ_JRS_batch.append(PZ_JRS)
        R_batch.append(R_temp)
    return PZ_JRS_batch, R_batch # ~, matrixpolyzono, ie, rotatotopes

def process_batch_JRS_trig_uncertain(jrs_tensor, q_0,qd_0,joint_axes):
    dtype, device = jrs_tensor.dtype, jrs_tensor.device 
    q_0 = q_0.to(dtype=dtype,device=device)
    qd_0 = qd_0.to(dtype=dtype,device=device)
    jrs_key = torch.tensor(JRS_KEY['c_kvi'],dtype=dtype,device=device)
    ...
    k_r = 10
    sin_E_p_g = torch.sin(ultimate_bound/k_r)
    cos_E_p_g = sin_E_p_g
    for i in range(n_joints):
        closest_idx = torch.argmin(abs(qd_0[i]-jrs_key))
        ...
        PZ_JRS.G[50:,0,acc_dim] = delta_breaking
        '''
        PZ_JRS.G[:,0,vel_dim] += ultimate_bound/k_r
        PZ_JRS.G[:,0,ka_dim] += 2*ultimate_bound
        R_temp= gen_batch_rotatotope_from_jrs_trig_uncertain(PZ_JRS,joint_axes[i],sin_E_p_g, cos_E_p_g)
